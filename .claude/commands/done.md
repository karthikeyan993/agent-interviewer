...
4. Calculate `time_taken_mins = (now - started_at) in minutes`

5. Read question from `data/questions/{topic}.json` → find by id

6. Read `data/config.json`

7. **Auto-compute confidence**:
   ```python
...
9. **Update question in topic file** (`data/questions/{topic}.json`):
...
13. **Update goals.json**: Increment `reviews_this_week`
